    ...
    for path, sub_dirs, files in os.walk(img_loc):
        for file in fnmatch.filter(files, "*.png"):
            ...
            if "non-radial" in file_path:
                img_s['non-radial'].update({img: img_s['non-radial'].get(img, []) + [cv2.imread(file_path)]})
            else:
                img_s['radial'].update({img: img_s['radial'].get(img, []) + [cv2.imread(file_path)]})
    return img_s


def cross_val_list():
    random.seed(1)
    non_radial_rand = random.sample([i for i in images['non-radial'].keys() if i not in list(images['radial'].keys())],
                                    k=images['radial'].__len__())
    dist = []
