...
base_path = "C:/Users/jahna/OneDrive/Documents/GitHub/Project/classfication_human_vs_non_human/data/cropped/input_image"
extension = '.png'
...
for file in os.listdir(base_path):
    print(file)
    if file == "human":
        file_path = base_path +"/human"
        for file2 in os.listdir(file_path):
            print(file2)
            image_path = os.path.join(file_path , file2)
     ...
     if file == "non-human":
        ...
        # image = load_img(image_path)
        # x = img_to_array(image, dtype=str)
        # x = x.reshape((1,) + x.shape)
        # X.append(x)
...
X = np.array(X, dtype=str).reshape(-1, 240, 240, 3)
X = X.astype('float32')
X = X / 255.
# split data
